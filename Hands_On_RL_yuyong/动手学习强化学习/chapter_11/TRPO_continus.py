...
class PolicyNetContinuous(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        std = F.softplus(self.fc_std(x)) + 1e-4  # 防止 std=0
        return mu, std
...
# ------------------- 训练工具函数（替代 rl_utils，防止依赖缺失） -------------------
def train_on_policy_agent(env, agent, num_episodes):
    return_list = []
    for i in range(10):
        with open(f'trpo_pendulum.log', 'w') as f:
            pass
    for i_episode in range(num_episodes):
        ...
        if (i_episode + 1) % 100 == 0:
            print(f'回合: {i_episode+1}, 平均奖励: {np.mean(return_list[-100:]):.2f}')
    return return_list
...
gamma = 0.9
lmbda = 0.9
critic_lr = 1e-3        # 降低学习率，更稳定
kl_constraint = 0.01  # 调大，允许策略正常更新
alpha = 0.5
